...
def generate_loop(path, depth, prev_var, path_from_networks):
    if len(path) == 1:
        ...
        for p in path_from_networks:
            ...
            ids += '{spaces}{resource_key} = meraki_network_{val}.net_{val}["${{domain.name}}/${{organization.name}}/${{network.name}}/{access_path}"].{resource_key}\n'.format(
                ...
                val="_".join([to_snake_case(i) for i in id_path]),
                access_path="/".join(
                    [
                        to_snake_case(i) + "/${" + to_snake_case(i)[:-1] + ".name}"
                        for i in id_path
                    ]
                ),
            )
    ...
    {spaces}data = try({prev_var}, null)""".format(
                ...
                prev_var=prev_var
                if path[0] == ""
                else prev_var + "." + to_snake_case(path[0]),
                ids=ids[:-1],
            ),
            ret_resource_keys,
        )
    ...
    if len(path) == 2:
        ...
        lbrace = "["
        rbrace = "]"
    content, ret_resource_keys = generate_loop(
        path[1:], depth + 1, local_var, path_from_networks
    )
    return (
        """
    {spaces}for {local_var} in try({prev_var}.{step}, []) : {lbrace}{spaces}{content}
    {spaces}{rbrace} if try({prev_var}.{step}, null) != null""".format(
            local_var=local_var,
            prev_var=prev_var,
            step=to_snake_case(step),
            content=content,
            spaces=" " * depth * 2,
            lbrace=lbrace,
            rbrace=rbrace,
...
def generate_template(resource, values, content, ret_resource_keys):
    ...
    for v in values:
        ...
        if v_terra not in ret_resource_keys:
            values_render += "  {v_terra} = try(each.value.data.{v_dots}, local.defaults.meraki.networks.{r}.{v_dots}, null)\n".format(
                v_terra=v_terra, v_dots=v_dots, r=resource
            )
    keys_render = ""
    ...
    return template.format(
        ...
        VALUES=values,
        values_render=values_render,
        content=content,
        keys_render=keys_render,
    )


with open(OPEN_API_SPEC_PATH) as f:
    j = json.loads(f.read())
...
res_name = "_".join(
    [to_snake_case(x) for x in spec_url.split("/") if not x.startswith("{")][1:]
)
path = ["domains", "organizations"] + path
path_from_networks = path[3:-1]
print(path_from_networks)
res_fields = fields(
    res[METHOD]["requestBody"]["content"]["application/json"]["schema"], []
)
