...
# Apply Organization-wide Settings for Login Security
locals {
  login_security = flatten([
    for domain in try(local.meraki.domains, []) : [
      for org in try(domain.organizations, []) : [
        for login in [try(org.login_security, {})] : {
          organization_id             = local.organization_map[org.name]
          enforce_password_expiration = try(login.enforce_password_expiration, local.defaults.meraki.organizations.login_security.enforce_password_expiration)
          password_expiration_days    = try(login.password_expiration_days, local.defaults.meraki.organizations.login_security.password_expiration_days)
          enforce_different_passwords = try(login.enforce_different_passwords, local.defaults.meraki.organizations.login_security.enforce_different_passwords)
          num_different_passwords     = try(login.num_different_passwords, local.defaults.meraki.organizations.login_security.num_different_passwords)
          enforce_strong_passwords    = try(login.enforce_strong_passwords, local.defaults.meraki.organizations.login_security.enforce_strong_passwords)
          enforce_account_lockout     = try(login.enforce_account_lockout, local.defaults.meraki.organizations.login_security.enforce_account_lockout)
          account_lockout_attempts    = try(login.account_lockout_attempts, local.defaults.meraki.organizations.login_security.account_lockout_attempts)
          enforce_idle_timeout        = try(login.enforce_idle_timeout, local.defaults.meraki.organizations.login_security.enforce_idle_timeout)
          idle_timeout_minutes        = try(login.idle_timeout_minutes, local.defaults.meraki.organizations.login_security.idle_timeout_minutes)
          enforce_two_factor_auth     = try(login.enforce_two_factor_auth, local.defaults.meraki.organizations.login_security.enforce_two_factor_auth)
          enforce_login_ip_ranges     = try(login.enforce_login_ip_ranges, local.defaults.meraki.organizations.login_security.enforce_login_ip_ranges)
          login_ip_ranges             = try(login.login_ip_ranges, local.defaults.meraki.organizations.login_security.login_ip_ranges)
        }
      ]
    ]
  ])
}
#  Issue - This loop will add a number to each duplicate key idx why is this required though ?
#  we have no key to match like name or org in other examples ?
#  I also appear to be pulling in defaults multiple times and if the defaults dont list all then error.
resource "meraki_organizations_login_security" "login_security" {
  for_each = { for idx, login in local.login_security : "${login.organization_id}_${idx}" => login }
  organization_id       = each.value.organization_id
