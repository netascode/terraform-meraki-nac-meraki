...
locals {
  networks_appliance_firewall_cellular_firewall_rules = flatten([
    for domain in try(local.meraki.domains, []) : [
      for organization in try(domain.organizations, []) : [
        for network in try(organization.networks, []) : {
          key        = format("%s/%s/%s", domain.name, organization.name, network.name)
          network_id = local.network_ids[format("%s/%s/%s", domain.name, organization.name, network.name)]
          rules = [
            for appliance_firewall_cellular_firewall_rule in try(network.appliance.firewall.cellular_firewall_rules, []) : {
...
resource "meraki_appliance_cellular_firewall_rules" "networks_appliance_firewall_cellular_firewall_rules" {
  for_each   = { for v in local.networks_appliance_firewall_cellular_firewall_rules : v.key => v }
  network_id = each.value.network_id
