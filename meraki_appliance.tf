      ...
      for organization in try(domain.organizations, []) : [
        for network in try(organization.networks, []) : [
          for appliance_vlan in try(network.appliance.vlans, []) : {
            ...
            group_policy_id = try(meraki_network_group_policy.networks_group_policies[format("%s/%s/%s/%s", domain.name, organization.name, network.name, try(appliance_vlan.group_policy_name, local.defaults.meraki.domains.organizations.networks.appliance.vlans.group_policy_name))].id, null)
