    ...
    for domain in try(local.meraki.domains, []) : [
      for organization in try(domain.organizations, []) : [
        for network in try(organization.networks, []) : {
          ...
          url_category_list_size = try(network.appliance.content_filtering.url_category_list_size, local.defaults.meraki.networks.appliance.content_filtering.url_category_list_size, null)
            ...
            for rule in try(network.appliance.firewall.one_to_many_nat_rules, []) : {
              ...
              port_rules = try(length(rule.port_rules) == 0, true) ? null : [
                for pr in try(rule.port_rules, []) : {
                  name        = try(pr.name, local.defaults.meraki.networks.appliance.firewall.one_to_many_nat_rules.port_rules.name, null)
    ...
    for domain in try(local.meraki.domains, []) : [
      for organization in try(domain.organizations, []) : [
        for network in try(organization.networks, []) : {
          ...
          mandatory_dhcp_enabled = try(network.appliance.single_lan.mandatory_dhcp, local.defaults.meraki.networks.appliance.single_lan.mandatory_dhcp, null)
